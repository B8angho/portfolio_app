...
const AnimationHome = () => {
  ...
  useEffect(() => {
    ...
    const globe = createGlobe(canvasRef.current, {
      ...
      markers: [
        // longitude latitude
        { location: [36.3504119, 127.3845475], size: 0.05 },
      ],
      onRender: (state) => {
  ...
  return (
    <>
      ...
      >
        <h1>I live in</h1>
        <button
          className="underline hover:text-red-500 dark:hover:text-cyan-500"
          onClick={() => {
            isClicked = true;
            focusRef.current = locationToAngles(36.3504119, 127.3845475);
          }}
        >
          <span className="custom-span">KOREA</span>
        </button>
      </div>
      <canvas
        ref={canvasRef}
        className="w-full h-full aspect-square cursor-grab transition-opacity"
      />
    </>
  );
};
